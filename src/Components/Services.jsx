      ...
      >
        {data.map((item) => (
          <Link
            ...
            key={item.id}
            className="relative overflow-hidden group bg-purple-300 px-5 py-10 flex flex-col items-center gap-2 rounded-lg hover:bg-purple-400 transition-all duration-200"
          >
            <div className="relative rotate-45 group-hover:rotate-0 transition-all duration-200 w-[4rem] h-[4rem] flex justify-center items-center bg-purple-800">
              <item.icon className="w-[2.5rem] -rotate-45 group-hover:rotate-0 transition-all duration-200 fill-white" />
            </div>
            <p className="mt-3 text-lg font-bold text-black">{item.title}</p>
